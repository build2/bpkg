...
namespace bpkg
{
  ...
  struct build_packages: build_package_list
  {
    ...
    // decide that a dependency is unused and collect it, potentially together
    // with some of its now unused recursive prerequisites, for dropping.
    // However, on some later refinement iteration some new dependent may be
    // introduced for this dependency. This will result in collecting this
    // dependency for build, overwriting the existing collection for drop. If
    // this dependency is not collected recursively (not being up/down-graded,
    // etc), then dropping of its own recursive prerequisites, if any, needs
