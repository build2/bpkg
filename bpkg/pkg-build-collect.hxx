...
namespace bpkg
{
  ...
  // Relations between dependent and dependency packages.
  //
  enum class dependent_relation
  {
    depends,   // The dependent directly depends on the dependency.
    constrains // The dependent constrains the potentially indirect dependency.
  };

  // Return 'constrains' if the dependent constrains the dependency.
  // Otherwise, return 'depends on' for a package and 'requires' for the
  // command line, etc.
  //
  inline string
  to_string (dependent_relation r, bool package = true)
  {
    return r == dependent_relation::constrains ? "constrains" :
           package                             ? "depends on" :
                                                 "requires";
  }

  inline ostream&
  operator<< (ostream& os, dependent_relation r) {return os << to_string (r);}
  ...
  struct build_package
  {
    ...
    // as empty dependency alternatives lists, which preserve the original
    // type (dependencies, constraint, etc). If present, it is parallel to the
    // available package's dependencies member.
    //
    ...
    // line. Also, specifies the relation type between the dependent and the
    ...
    struct constraint
    {
      // If the dependent relation is 'depends', then originates from the
      // dependent's depends manifest value (dependent's version is present)
      // or from the build-to-hold command line spec. If the dependent
      // relation is 'constrains', then originates from the dependent's
      // constrains manifest value or from the dependency command line spec.
      //
      ...
      //
      bpkg::dependent_relation dependent_relation;
      ...
      // Return the dependency constraint string representation. For example:
      //
      ...
      operator== (const constraint& v) const
      {
        return version_constraint == v.version_constraint &&
               ...
               dependent_relation == v.dependent_relation;
      }
    };
    ...
    // clauses is not triggered by the presence of the global configuration
  ...
  //
  // When we recursively collect the prerequisites for a package build, along
  // with the package dependencies we also collect the dependency constraints
  ...
  // satisfactory dependency version and the dependency configuration
  // negotiation machinery. We, however, only need to consider a dependency
  ...
  // the constraints' active/inactive states on the first iteration of the
  // collection. Then, after the plan execution is simulated, we deduce the
  ...
  // states. After the plan execution simulation we re-check the used states
  // and, if any of them don't match the actual state, correct them and
  // re-collect again. Note, however, that we may potentially end up yo-yoing
  // due to the fact that an active constraint potentially can push out the
  // dependency from the dependent's dependency tree by applying the version
  // constraint, reflecting, etc and the now deactivated constraint can allow
  // the dependency back into the tree. We will detect the repeated attempt to
  // apply specific constraints states change to the same collection initial
  // state, issue diagnostics, and fail.
  //
  ...
  struct dependency_constraint_key
  {
    ...
    dependency_constraint_key (database& d,
                               package_name dpt,
                               version v,
                               package_name dep,
                               bool bt)
        : dependent (d, move (dpt), move (v)),
          dependency (move (dep)),
          buildtime (bt) {}
    ...
    operator< (const dependency_constraint_key& v) const
    {
      if (dependent != v.dependent)
        return dependent < v.dependent;

      if (dependency != v.dependency)
        return dependency < v.dependency;

      return buildtime < v.buildtime;
    }

    // Return the dependency constraint string representation in the form:
    //
    ...
    to_checksum (xxh64& cs) const
    {
      dependent.to_checksum (cs);
      cs.append (dependency.string ());
      cs.append (buildtime);
    }
  };
  ...
  //
  string
  to_string (dependency_constraint_state);

  inline ostream&
  operator<< (ostream& os, const dependency_constraint_state& s)
  {
