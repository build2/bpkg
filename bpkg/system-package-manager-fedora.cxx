...
namespace bpkg
{
  ...
  dnf_list (vector<package_info>& pis, bool modify_system, size_t n)
  {
    ...
    // the question if all the repositories metadata is cached or not. Thus,
    // we will just run dnf-list with the --cacheonly option for the rpm
    // package, redirecting all the streams to /dev/null. If this command
    // fails, we assume that the repositories metadata is not cached. If we
    // are wrong and it fails for some other reason, then something is
    // probably severely damaged and the subsequent dnf-list command without
    // --cacheonly option will likely to fail either.
    //
