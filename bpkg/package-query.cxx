...
namespace bpkg
{
                   ...
                   bool revision)
  {
    ...
    // doesn't feel to be overly big (~100). Thus, we will cache such query
    // variations as they appear, calculating their names accordingly.
    //
    string qn (db.uuid.string () + "-available-package-query");

    // Return the query name component based on the fact if the constraint's
    // endpoint version comparison is revision/iteration sensitive or not.
    //
    auto qnr = [] (bool revision, bool iteration) -> string
    {
    ...
    if (c)
    {
      ...
      {
        qn += "-eq" + qnr (revision || c->min_version->revision, revision);
      }
      else
      {
        if (c->min_version)
        {
          qn += (c->min_open ? "-min-gt" : "-min-ge") +
                qnr (revision || c->min_version->revision, revision);
        }

        if (c->max_version)
        {
          qn += (c->max_open ? "-max-lt" : "-max-le") +
                qnr (revision || c->max_version->revision, revision);
        }
      }
    }
      ...
      if (c)
      {
        ...
        else
        {
          ...
          if (c->max_version)
          {
            ...
            if (c->max_open)
              qr = qr && compare_version_ref_lt (vm, cv, rv, revision);
            else
              qr = qr && compare_version_ref_le (vm, cv, rv, revision);
          }
