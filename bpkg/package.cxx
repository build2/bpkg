...
namespace bpkg
{
  ...
  to_checksum (xxh64& cs) const
  {
    ...
    if (version)
      cs.append (!version->empty () ? version->string () : empty_string);
  }
  ...
  */
  ...
  //
  string
  to_string (prerequisite_type t)
  {
    switch (t)
    {
    case prerequisite_type::constraint: return "constraint";
    case prerequisite_type::dependency: return "dependency";
    }
  ...
  prerequisite_type
  to_prerequisite_type (const string& t)
  {
         if (t == "dependency") return prerequisite_type::dependency;
    else if (t == "constraint") return prerequisite_type::constraint;
    else throw invalid_argument ("invalid prerequisite type '" + t + '\'');
  }
  ...
  // Cache of flags (values), indicating if the specified package is an
  // indirect prerequisite of a specific kind (build-time, runtime, or any;
  // second part of key) for a specific dependent (first part of key).
  //
  using indirect_prerequisite_cache =
    map<pair<shared_ptr<selected_package>, optional<bool>>, bool>;

  static bool
  indirect_prerequisite (const package_name& name,
                         optional<bool> buildtime,
                         const package_prerequisites& prereqs,
                         const function<find_package_prerequisites_function>& f,
                         indirect_prerequisite_cache& cache)
  {
    ...
    for (const auto& pr: prereqs)
    {
      ...
      if (pr.first.object_id () == name            &&
          pi.type == prerequisite_type::dependency &&
    ...
    for (const auto& pr: prereqs)
    {
      ...
      if (pi.type == prerequisite_type::dependency)
      {
        ...
        auto prereq = [&p, &name, &f, &cache] (optional<bool> buildtime)
        {
          ...
          bool r (indirect_prerequisite (name, buildtime, *pp, f, cache));
          cache[k] = r;
          return r;
        };
        ...
        else                  // Build-time.
        {
          ...
          // prerequisite in the prerequisite's tree and bail out if the
          // search succeeds. Otherwise, if this is a build-time prerequisite,
          // then search for the prerequisite of any kind in the
          // prerequisite's tree.
          //
  ...
  {
    indirect_prerequisite_cache cache;
    return indirect_prerequisite (name, buildtime, prereqs, f, cache);
  }
