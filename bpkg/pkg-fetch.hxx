...
namespace bpkg
{
  ...
  // keep_transaction_if_safe is false or keeping it deemed unsafe. Return the
  ...
  // an existing archive to the selected package (while using external archive
  // or cached archive when src caching is enabled) and, if replacing, don't
  // purge the current archive. In this case, if the transaction is aborted
  // after the function call, the database and filesystem states stay
  ...
  // transaction, if keep_transaction_if_safe is false or keeping it deemed
