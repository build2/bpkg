...
namespace bpkg
{
  ...
  // Dependency which needs to be added/replaced in the build plan in
                       ...
                       bool ignore_unsatisfiable)
  {
    ...
    // pkg-build/dependency/constraints/constrains/from-scratch/reflect-require
  ...
  pkg_build (const pkg_build_options& o, cli::group_scanner& args)
  {
    ...
    build_packages pkgs;
    {
      ...
      // modification or the mode change (see above refinement, cmdline_deny*,
      // etc for details). Never reset. Used for calculating checksum of the
      // collection initial state (see below).
      //
           ...
           refine; )
      {
        ...
        to_checksum (collection_initial_state, dependency_constrs);
        ...
        // then update non-matching states and re-collect from scratch.
        //
        assert (refine && !scratch_col);

        if (!dependency_constrs.empty ())
        {
          scratch_col =
