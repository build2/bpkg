...
namespace bpkg
{
  ...
  // own dependencies anymore and also its non-shared dependencies are
  // gone from the map, recursively (see replaced_versions for more details).
  ...
  struct build_package
  {
    ...
    // list. This, in particular, allows not to re-evaluate conditions
  ...
  // for details). Thus, if after the package collection some bogus entries
  ...
  public:
    ...
    void
    add_shadow (package_key dependent, pair<size_t, size_t> pos)
    {
    ...
    bool
    contains_shadow_dependent (package_key dependent,
  ...
  // Map of existing dependents which need to be re-evaluated to the position
  // with the dependency index not greater than the specified one.
  //
  ...
  // dependency index was performed. Thus, if after the package collection
  ...
  struct build_packages: build_package_list
  {
    ...
    // postpone_dependency exception. This exception is handled by
                             ...
                             postponed_configuration* pcfg = nullptr)
    {
      ...
      // to be postponed. This is normally required if this cluster contains
      // some existing dependent which needs to be re-evaluated to a
      ...
      // the information required for such a creation (see
      // postponed_position's replace flag for details).
      //
      // @@ Maybe it's better to call it postpone_configuration or similar?
      //
      ...
      if (pcfg != nullptr)
      {
        ...
        //
        {
          ...
          //
          // As it was said, we may end up adding additional dependencies to
              ...
              for (auto& d: dependents)
              {
                ...
                for (const postponed_configuration& cfg: postponed_cfgs)
                {
                      ...
                      cfg.existing_dependent_position (pk))
                  {
                    ...
                    {
                      // Otherwise, this dependent wouldn't be considered as
                      // an existing by query_existing_dependents() since as
                      // it is (being) negotiated then it is already
                      // re-evaluated and so is being built (see the verify
                      // lambda above).
                      //
                      assert (ei > di);
  ...
  private:
    ...
    // dependent package to the resulting list, returning true in that's case.
    //
      ...
      postponed_configurations& postponed_cfgs)
    {
      ...
      // query_existing_dependents(). If that's wasn't the case, the below
      // would likely fall apart for it.
      //
