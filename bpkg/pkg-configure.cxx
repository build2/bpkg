...
namespace bpkg
{
    ...
    const vector<package_key>* unconstrain_deps)
  {
    ...
    for (size_t di (0); di != deps.size (); ++di)
    {
      ...
      // Go straight to the "make dependency decisions" mode for single
      // alternative.
      //
             ...
             edas.size () != 1 ? prev_prereqs : nullptr);;)
      {
        ...
        for (const auto& eda: edas)
        {
          ...
          for (; i != e; ++i)
          {
            ...
            // If the dependency is not selected, then skip it.
            //
            ...
            // If the dependency is not configured, then skip it.
            //
            ...
            prerequisites.emplace_back (
              ...
              prerequisite_info {(constraint
                                  ? prerequisite_type::constraint
                                  : prerequisite_type::dependency),
                                 *dc,
                                 buildtime, !buildtime});
          }
          ...
          for (auto& pr: prerequisites)
          {
            ...
            if (!p.second)
            {
              ...
              //
              if (pie.type == prerequisite_type::constraint &&
                  pi.type  == prerequisite_type::dependency)
              {
                pie.type = prerequisite_type::dependency;
        ...
        // If no dependency alternative is selected, then, if this is a
        // dependency constraint, assume it inactive and skip. Otherwise
        // (regular dependency) fail, unless we are in the "recreate
        // dependency decisions" mode. In the latter case fall back to the
        // "make dependency decisions" mode and retry.
        //
