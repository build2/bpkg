...
namespace bpkg
{
  // build_package::constraint
  //
  std::string build_package::constraint::
  string (const package_name& dependency) const
  {
    std::string r (dependent.string ());
    r += ' ';
    r += to_string (dependent_relation, dependent.version.has_value ());
    r += " (";
    r += dependency.string ();
    r += ' ';
    r += version_constraint.string ();
    r += ')';
    return r;
  }
  ...
  // dependency_constraint_key
  //
  string dependency_constraint_key::
  string () const
  {
    std::string r (dependent.string ());
    r += " : ";
  ...
  //
  string
  to_string (dependency_constraint_state s)
  {
    switch (s)
    {
  ...
  correct_states (const xxh64& collection_initial_state)
  {
    ...
    vector<dependency_constraints::value_type*> corrected;
    vector<const dependency_constraints::value_type*> cycled;
    ...
    for (auto& dc: *this)
    {
      const dependency_constraint_key& dck (dc.first);
      const package_version_key& dpt (dck.dependent);
          ...
          sp->substate != package_substate::system)
      {
        ...
        if (*dpt.version == sp->version)
        {
          ...
          dependency_constraint_state& s (dc.second);
          ...
          if (assumed_active != active)
          {
            ...
            {
              l5 ([&]{trace << "dependency constraint (" << dck << ") is "
                            << "wrongfully assumed " << s << ", correcting";});
              ...
              //
              dck.to_checksum (cs);
              cs.append (static_cast<uint8_t> (s));
    ...
    if (!corrected.empty ())
    {
      ...
      {
        for (dependency_constraints::value_type* dc: corrected)
        {
          dc->second = dependency_constraint_state::version_constraint_only;

          l5 ([&]{trace << "dependency constraint (" << dc->first << ") is "
                        << "assumed " << dc->second << " to prevent "
    ...
    if (!cycled.empty ())
    {
      ...
      for (const dependency_constraints::value_type* dc: cycled)
        dr << info << "involved dependency constraint (" << dc->first << ')';
    }
  ...
  clear (dependency_constraint_state s)
  {
       ...
       vector<package_key>&& dc)
  {
    if (unsatisfied_dependent* ud = find_dependent (dpt))
    {
      ...
      if (find_if (ics.begin (), ics.end (),
          ...
          ics.end ())
      {
        ics.emplace_back (dep, c, rel, move (ucs), move (dc));
      }
    }
    else
      push_back (
        unsatisfied_dependent {
          dpt, {ignored_constraint (dep, c, rel, move (ucs), move (dc))}});
  }
                               ...
                               const optional<package_key>& orig_dep)
  {
    ...
    for (size_t di (sdeps.size ()); di != deps.size (); ++di)
    {
        ...
        for (size_t i (0); i != das.size (); ++i)
        {
          ...
          if (enabled && constraint)
          {
            ...
            if (i != dependency_constrs.end ())
            {
              ...
              l5 ([&]{trace << "dependency constraint (" << k << ") is "
                            << "assumed " << s << " as per cache";});
            }
            else
            {
              if (sp != nullptr                          &&
                  ...
                  sp->substate != package_substate::system)
              {
                ...
                active = (
                  find_if (
                    ...
                    [&k] (const auto& pr)
                    {
                      return pr.first.object_id () == k.dependency &&
                             (k.buildtime
                              ? pr.second.buildtime
                              : pr.second.runtime);
                    }) != prs.end ());
                ...
                l5 ([&]{trace << "dependency constraint (" << k << ") is "
                              << "assumed " << (active ? "active" : "inactive")
                              << " as per configured dependent "
                              << sp->string (pdb);});
              }
              else
              {
                ...
                l5 ([&]{trace << "dependency constraint (" << k
                              << ") is assumed inactive initially";});
              }

              dependency_constrs[move (k)] =
                (active
                 ? dependency_constraint_state::active
                 : dependency_constraint_state::inactive);
            }
      ...
      // gets selected, then this unsatisfied dependent/constraints are
          ...
          for (auto di (da.begin ()); di != da.end (); )
          {
                  ...
                  !bp.constraints.empty ())
              {
                    ...
                    !satisfies (c.version_constraint, dp.constraint))
                {
                  ...
                  if (!ignore_unsatisfactory_dep_spec)
                  {
                    if (dr != nullptr)
                    {
                      ...
                      *dr << error << "unable to satisfy constraints on package "
                          << dn <<
                        info << nm << pdb << ' '
                             << (constraint ? "constrains" : "depends on")
                             << " (" << dn << ' ' << *dp.constraint << ')';
                  ...
                  else
                  {
                    ...
                    dependent_relation rel (constraint
                                            ? dependent_relation::constrains
                                            : dependent_relation::depends);
            ...
            if (dap == nullptr)
            {
              ...
              if (dap == nullptr)
              {
                ...
                if (!aps.empty ())
                {
                  dr << "unable to satisfy "
                     << (!dep_constr ? "dependency" : "command line")
                     << " constraint (" << dn;
            ...
            optional<build_package::constraint> dc (
              dp.constraint
      ...
      auto collect = [&options,
         ...
         bool ignore_unsatisfactory_dep_spec)
        {
          ...
          for (prebuild& b: bs)
          {
            ...
            //
            vector<build_package::constraint> constraints;

            if (b.effective_constraint)
            {
              constraints.push_back (*b.effective_constraint);
    ...
    for (const auto& rd: rpt_depts)
    {
...

      {
        ...
        if (i != map_.end ())
        {
          ...
          // @@ The decision to just keep the package build just pre-entered
          //    doesn't work well, since we will just crash later trying to
          //    order pre-entered build (no action). Note that skipping such a
          ...
          //    from the top of my head. A possible fix direction can be
  ...
  cancel_drop_prerequisites (const selected_package& sp, database& db)
  {
    ...
    for (const auto& pr: sp.prerequisites)
    {
      ...
      if (i != map_.end ())
      {
        ...
        {
          // Remove the map entry before the recursive
          // cancel_drop_prerequisites() call, not to traverse through the
          // same prerequisite multiple times via different dependents.
          //
          shared_ptr<selected_package> dsp (move (bp.selected));

          // By definition (see build_package::action_type::drop for details).
          //
          assert (dsp != nullptr);
                                   ...
                                   set<package_key>& visited_deps)
  {
    ...
    for (database& ddb: pdb.dependent_configs (true /* sys_rep */))
    {
      for (auto& pd: query_dependents_cache (ddb, name, pdb))
      {
        ...
        // and, if so, schedule it for re-collection. Afterwards, scheduled or
        // not, collect its own constraining dependents, recursively.
        //
        ...
        if (i != map_.end ())
        {
          ...
          // Don't check dependents configured as system and those which
        ...
        // has_constraint flag. However, given that constraining dependents
        // are quite rare, most of such loads will be a waste. Thus, we will
        // only load the selected package if its has_constraint flag is true.
        //
        if (check)
        {
        ...
        if (check)
        {
          ...
          if (!satisfies (av, c))
          {
            ...
            unsatisfied_depts.add (d,
                                   ...
                                   c,
                                   (pd.type == prerequisite_type::constraint
                                    ? dependent_relation::constrains
                                    : dependent_relation::depends));
          }
        }
    ...
    //
    {
      ...
      if (i->second.version_constraint)
        p.constraints.emplace_back (
           *i->second.version_constraint,
           dpt.db,
           dpt.name,
           *dpt.version,
           (i->second.type == prerequisite_type::constraint
            ? dependent_relation::constrains
            : dependent_relation::depends),
