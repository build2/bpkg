           ...
           ((flags & adjust_reconfigure) != 0 ||
            (*action == build &&
             (selected->system () != system             ||
              selected->version != available_version () ||
              replace ()                                ||
              (!system && (configuration_specified () || disfigure)))));
  }
                           ...
                           postponed_configuration* pcfg)
  {
    ...
    if (pcfg != nullptr)
    {
                ...
                e (pcfg->dependents.end ()); i != e; )
      {
        ...
        // were encountered (see the recollect_existing_dependents exception
        // throw sites for details). In the latter case we just skip the
        // dependent for now, expecting it to be re-collected some time later.
        // For a dependency, it should not already be there (since we haven't
        // yet started recursively collecting it). But we could be
