...
namespace bpkg
{
                      ...
                      bool iu)
  {
    ...
    // the manifest into memory and doesn't print any progress indication,
    // which can be confusing for large packages.manifest files. There is no
    // easy way to change this function to print the proper progress
    // indication (the file name specifically) when using wget and fetch as
    // the underlying fetch programs. Thus, we fetch packages.manifest into a
    // temporary directory and remove it after the parsing.
    //
    if (rl.remote ())
    {
