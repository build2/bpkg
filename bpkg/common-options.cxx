...
namespace bpkg
{
  ...
  print_long_usage (::std::ostream& os, ::bpkg::cli::usage_para p)
  {
    ...
    os << std::endl
       ...
       << "                          currently recognized modes are \033[1mno-src\033[0m, \033[1mno-trust\033[0m, and" << ::std::endl
       << "                          \033[1moffline\033[0m, with their semantics described below. The" << ::std::endl
       << "                          local cache mode can also be specified with the" << ::std::endl
       << "                          \033[1mBPKG_FETCH_CACHE\033[0m environment variable. Its value has" << ::std::endl
       << "                          the same format as this option." << ::std::endl
       << ::std::endl
       << "                          The \033[1mno-src\033[0m mode disables sharing of source" << ::std::endl
       << "                          directories for dependencies from the local fetch" << ::std::endl
       << "                          cache, instead causing private copies to be made in" << ::std::endl
       << "                          the build configuration directories. Note that this" << ::std::endl
       << "                          behavior is the default if the fetch cache is" << ::std::endl
       << "                          disabled (\033[1m--no-fetch-cache\033[0m)." << ::std::endl
       << ::std::endl
       << "                          The \033[1mno-trust\033[0m mode disables caching of the repository" << ::std::endl
       << "                          authentication answers given by the user in the local" << ::std::endl
       << "                          fetch cache. See \033[1mbpkg-repository-signing(1)\033[0m for" << ::std::endl
       << "                          background." << ::std::endl
       << ::std::endl
    ...
    os << std::endl
       ...
       << "                          variable." << ::std::endl
       << ::std::endl
       << "                          If not specified, then local cache is placed into the" << ::std::endl
       << "                          \033[1m.build2/cache/\033[0m subdirectory of the user's home" << ::std::endl
       << "                          directory. Note that the cache is not placed into" << ::std::endl
       << "                          \033[1m$HOME/.cache/\033[0m or equivalent because, unless" << ::std::endl
       << "                          \033[1m--no-shared-src\033[0m is specified, it is semi-precious:" << ::std::endl
       << "                          while you won't loose any information if the cache is" << ::std::endl
       << "                          removed, you may end up with broken builds that will" << ::std::endl
       << "                          require manual repair." << ::std::endl
       << ::std::endl
       << "                          Note also that while there is currently no way to" << ::std::endl
       << "                          impose a size limit on the cache, outdated cache" << ::std::endl
       << "                          entries are periodically removed." << ::std::endl;
